...
class Compactor(ApetureOptimizer):
    ...
    def build_Lattice(self):
        ...
        self.lattice.add_Lens_Sim_With_Caps(None,'collectorMagnet.txt',2.0,None)
    ...
    def trace_Through_Compactor(self,parallel=False):
        ...
        return swarm
    def get_Width_Along_Lattice(self,swarm,numPoints,elStart=0,fraction=.9):
        zArr=np.linspace(self.lattice.elList[elStart].r1[0]+1e-6,self.lattice.elList[-1].r2[0]-1e-6,num=numPoints)
        def func(z):
            width=self.get_Width_At_Point(swarm,z,fraction=fraction)
            return width
        widthList=self.helper.parallel_Problem(func,argsList=zArr,onlyReturnResults=True)
        return zArr,np.asarray(widthList)
    ...
    def initialize_Test_Swarm2(self):
        #A swarm originationg from the origin but spread out in phase space
        print('using test swarm 2')
        swarm=Swarm()
        numTheta=10
        numPhiParticleMax=250
        pMax=5.0
        thetaMax=pMax/self.v0Nominal
        thetaArr=[thetaMax]#np.linspace(thetaMax/5,thetaMax,num=numTheta)
        pxArr=[0]#np.linspace(-1,1,num=3)
        colorList=['blue','green','orange']
        for theta in thetaArr:
            numPhi=int(numPhiParticleMax*theta/thetaMax)
            ...
            for phi in phiArr:
                ...
                for pxDelta in pxArr:
                    ...
                    i+=1
        self.swarmInitial=swarm
    def initialize_Test_Swarm3(self,rMax=.005,pTransMax=15.0,numParticles=1000,T=.001,seedDistance=0.0):
        ...
        radius=(.625/numParticles)**.25 #volume of 4d sphere goes as r**4
        samplesArr=np.asarray(poisson_disc.Bridson_sampling(np.asarray([1,1,1,1]),radius))
        samplesArr=2*(samplesArr-.5)
        samplesArr[:,:2]=samplesArr[:,:2]*rMax
        samplesArr[:,2:]=samplesArr[:,2:]*pTransMax
        swarm=Swarm()
        for sample in samplesArr:
            qy,qz,py,pz=sample
            if np.sqrt(qy**2+qz**2)<=rMax:
                if np.sqrt(py**2+pz**2)<=pTransMax:
                    #walk the particle up to the plane
    ...
    def find_Image_Plane(self):
        self.initialize_Test_Swarm2()
        swarm = self.trace_Through_Compactor(parallel=False)
        zArr,widthArr=self.get_Width_Along_Lattice(swarm,1000,elStart=1)
        ...
        # plt.title('90% width versus distance along optical axis starting \n at lens output')
        # plt.axvline(x=zFocal,c='black')
        # plt.axvline(x=self.lattice.elList[1].r2[0],c='black')
        # # plt.text(.4,.02,'Focus')
        # # plt.text(zCutoff,.02,'Lens output')
    ...
    def get_Width_At_Point(self,swarm,z0,fraction=.9):
        ...
        width = self.get_Frac_Width(np.asarray(qList), fraction=fraction)
        return width
...
compactor.X = {"Lo1": .6, "Lm1": .25, "rp1": .05,'ap1':1.0, 'Lsep1': 1.75, 'Lm2': .171 ,'rp2': .0241,
