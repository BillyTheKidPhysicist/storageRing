...
@numba.njit()
def _force_Func_Internal(x, y, z, field_data_internal):
...
@numba.njit()
def _force_Func_External(x, y, z, field_data_external):
    x_arr, y_arr, z_arr, FxArr, FyArr, Fz_arr, V_arr = field_data_external
    Fx, Fy, Fz = vec_interp3D(x, y, z, x_arr, y_arr, z_arr, FxArr, FyArr, Fz_arr)
    return Fx, Fy, Fz


@numba.njit()
def _magnetic_potential_Func_Internal(x, y, z, field_data_internal):
    x_arr, y_arr, z_arr, FxArr, FyArr, Fz_arr, V_arr = field_data_internal
    return scalar_interp3D(x, y, z, x_arr, y_arr, z_arr, V_arr)


@numba.njit()
def _magnetic_potential_Func_External(x, y, z, field_data_external):
    x_arr, y_arr, z_arr, FxArr, FyArr, Fz_arr, V_arr = field_data_external
    return scalar_interp3D(x, y, z, x_arr, y_arr, z_arr, V_arr)
...
def force_Without_isInside_Check(x0, y0, z0, params, field_data):
    ...
    field_data_internal, field_data_external = field_data
    x, y, z = x0, y0, z0
    ...
    if use_symmetry:
        ...
        if -extra_field_length <= x <= space:
            ...
            Fx, Fy, Fz = _force_Func_External(x, y, z, field_data_external)
        elif space < x <= symmetryPlaneX:
            Fx, Fy, Fz = _force_Func_Internal(x, y, z, field_data_internal)
...
def magnetic_potential(x, y, z, params, field_data):
    ...
    if use_symmetry:
        if -extra_field_length <= x <= space:
            V = _magnetic_potential_Func_External(x, y, z, field_data_external)
        elif space < x <= symmetryPlaneX:
            V = _magnetic_potential_Func_Internal(x, y, z, field_data_internal)
        elif symmetryPlaneX < x <= Lm + space:
            x = 2 * symmetryPlaneX - x
            V = _magnetic_potential_Func_Internal(x, y, z, field_data_internal)
