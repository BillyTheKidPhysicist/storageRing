"""
Functions and classes to aid in analysis of collector lens, the big 6 layer lens after the nozzle
"""
...
def interpolate_values(x_interp, qf_vals, pf_vals, vT_max, max_radius) -> tuple[
...
class CollectorSwarmAnalyzer:
    """
    Class to assist in analysis of the collector lens primarily by interpolation. This class works by interpolating
    over the last element with particle's final position and momentum used to project coord backwards
    """
    ...
    def interpolate(self, x_interp: float, max_radius_mm: float = np.inf, laser_scan_range_volts: float = None,
                    vtrans_max: float = None,
                    return_p: bool = False, return_valid_indices: bool = False, enforce_location: bool = True) -> list[
        np.ndarray, ...]:
        if enforce_location:
        ...
        y_vals, z_vals, p_vals, valid_indices = interpolate_values(x_interp, self.qf_vals, self.pf_vals, vTMax,
                                                                   max_radius_meter)
    ...
    def transvers_position_density(self, x_interp: float, r_max: float, bins_center_sep_mm=.1) -> tuple[
    ...
    def transverse_speed_density(self, x_position: float, speed_trans_max: float = np.inf, r_max: float = np.inf,
                                 bins_center_sep_mm=.1) -> tuple[np.ndarray, np.ndarray]:
        """Return coordinates (radial values) and density of transverse particles distribution"""
        speed_trans = self.transverse_speeds(x_position, speed_trans_max, r_max)
